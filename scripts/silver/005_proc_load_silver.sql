...
Script Purpose:
    ...
    - Performs data cleaning and transformation on data from the bronze layer, then loads this data into the silver layer tables.
...
BEGIN
	...
	WHERE t.flag_last = 1; -- Select most recent record per customer
	...
	SELECT 
		...
		sls_prd_key,
		sls_cust_id,
		CASE WHEN sls_order_dt IS NULL OR sls_order_dt = 0 OR LENGTH(sls_order_dt::TEXT) != 8 THEN NULL 
			 ELSE TO_DATE(sls_order_dt::TEXT, 'YYYYMMDD') 
		END AS sls_order_dt,
		CASE WHEN sls_ship_dt IS NULL OR sls_ship_dt = 0 OR LENGTH(sls_ship_dt::TEXT) != 8 THEN NULL 
			 ELSE TO_DATE(sls_ship_dt::TEXT, 'YYYYMMDD') 
		END AS sls_ship_dt,
		CASE WHEN sls_due_dt IS NULL OR sls_due_dt = 0 OR LENGTH(sls_due_dt::TEXT) != 8 THEN NULL 
		...
		CASE WHEN sls_price = 0 OR sls_price IS NULL THEN sls_sales / NULLIF(sls_quantity,0)
			 ...
			 ELSE sls_price -- Derive price if original sales price is invalid
		END AS sls_price
